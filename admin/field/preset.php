...
class JFormFieldPreset extends JFormField
{
	...
	protected function getInput()
	{
		...
		// Blank one
		$options[] = array(
			'text' => JText::_('TPL_STAR_SELECT_PRESET'),
			'value' => ''
			);
		// Preset group
		$options[] = array(
			'text' => JText::_('TPL_STAR_PRESET_GROUP_PRESET'),
			'value' => '<OPTGROUP>'
			);
		$i = 0;
		...
		// Last saved group
		$options[] = array(
			'text' => JText::_('TPL_STAR_PRESET_GROUP_REVISIONS'),
			'value' => '<OPTGROUP>'
			);
		// load from revisons
		$style_id = JFactory::getApplication()->input->getInt('id');
		$helper = require ((dirname(dirname(__DIR__))) . '/helper.php');
		$helper->loadStyle ($style_id);
		// Check if template style is just saved and run once
		$helper->afterSave();

		$path = JPATH_ROOT . $helper->getTemplateLocation() . '/css/revisions/' . $this->element['group'] . '/';
		// saved revision
		...
		$html = JHtml::_('select.genericlist', $options, '', array('option.attr'=>'attr', 'groups' => true, 'list.attr'=>'class="preset-loader"'));
		return $html;
	}
